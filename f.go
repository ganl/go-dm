...
func Compress(srcBuffer *Dm_build_1274, offset int, length int, compressID int) ([]byte, error) {
	if compressID == Dm_build_360 {
		return snappy.Encode(nil, srcBuffer.Dm_build_1568(offset, length)), nil
	}
...
func GzlibCompress(srcBuffer *Dm_build_1274, offset int, length int) ([]byte, error) {
	var ret bytes.Buffer
	var w = zlib.NewWriter(&ret)
	w.Write(srcBuffer.Dm_build_1568(offset, length))
	w.Close()
